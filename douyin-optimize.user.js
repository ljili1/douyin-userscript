...
(function (_whitesev_domutils, _whitesev_pops, _whitesev_utils, qmsg) {
  ...
  var BlockLeftNavigator = {
    init() {
      ...
      Panel.execMenuOnce("hide-leftNav-on-hover", () => {
        return this.hideLeftNavigatorOnHover();
      });
    },
    shieldLeftNavigator() {
      log.info("【屏蔽】左侧导航栏");
  ...
  var BlockTopNavigator = {
    init() {
      ...
      Panel.execMenuOnce("hide-topNav-on-hover", () => {
        return this.hideTopNavigatorOnHover();
      });
    },
    shieldTopNavigator() {
      log.info("【屏蔽】顶部导航栏");
    ...
    removeStyleBottom() {
      ...
      const lockFn = new utils$1.LockFunction(() => {
        queryDeep("xg-video-container, .douyin-player-video-container").forEach(($el) => {
          if ($el.style.getPropertyPriority("bottom") === "important" && $el.style.getPropertyValue("bottom") === "0px") return;
