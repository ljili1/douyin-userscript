...
| 功能 | 说明 |
|---|---|
| 隐藏左侧导航栏（悬停显示） | 左侧导航栏默认隐藏，鼠标移入左侧边缘 **12px** 窄条自动滑出、移出自动收起；导航改为 `fixed` 悬浮层，脱离 flex 布局不再占位 |
| 隐藏顶部导航栏（悬停显示） | 顶部导航栏默认隐藏，鼠标移入顶部 **12px** 区域自动显示；并修复隐藏后「消息」面板顶部留白问题 |
...
- 定制调整（2026-09-19）：左侧导航悬停触发区由 30px 收窄至 12px，与顶部导航触发高度对齐，减少误触发
